...
WHERE individual_export.modified_by IN ('System', 'Laura Haverkamp')
  ...
  AND (individual_export.middle_name <=> t.middle_name OR t.middle_name IS NULL)
...
--  AND individual_export.other_zip <=> t.address_other_postal_code
  ...
  AND (individual_export.email_work <=> t.email_work OR t.email_work IS NULL)
  AND (individual_export.maiden_name <=> t.maiden_name OR t.maiden_name IS NULL)
  AND (individual_export.first_communion <=> t.first_communion_date OR t.first_communion_date IS NULL)
  ...
  AND (individual_export.newsletter <=> IF(t.newsletter, 'Yes', 'No') OR individual_export.newsletter IS NULL)
  AND (individual_export.confirmed <=> IF(t.confirmed, 'Yes', 'No') OR individual_export.confirmed IS NULL)
